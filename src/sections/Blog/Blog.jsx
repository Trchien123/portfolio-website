  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-4" data-aos="fade-up">
        <div>
          <h2 className="text-4xl md:text-5xl font-bold text-sage mb-4 flex items-center gap-3">
            <BookOpen className="text-neon" size={40} />
            Blogs
          </h2>
          <p className="text-sage/70 text-md max-w-2xl">
            Sharing my journey in <span className="text-neon font-bold">Artificial Intelligence</span> and <span className="text-sage font-bold">Software Development</span>.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-8">
        {posts.map((post, index) => (
          <article 
            key={post.id} 
            className="group cursor-pointer bg-navy/30 border border-white/10 rounded-2xl flex flex-col md:flex-row hover:border-neon/50 hover:-translate-y-1 transition-all duration-300 shadow-lg relative overflow-hidden h-auto md:h-52"
          ...
          >
            ...
            <div className="group w-full flex flex-col p-5">
              ...
              {/* Hover Effect */}
              <div className="absolute top-0 right-0 w-32 h-32 bg-neon/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                ...
                <div className="flex justify-between items-start mb-3">
                    <div className="flex flex-wrap gap-2 relative z-10">
                        {post.tags.map((tag, idx) => ( 
                            <span key={idx} className="text-sm font-mono font-bold text-neon bg-neon/10 px-2 py-0.5 rounded border border-neon/20">
                                {tag}
                            </span>
                        ))}
                    </div>
                    {/* Date */}
                    <div className="flex items-center gap-3 text-sm text-sage/50 relative z-10">
                        <span className="flex items-center gap-1"><Calendar size={12}/> {post.date}</span>
                    </div>
                </div>

                {/* Title */}
                <h3 className="text-2xl md:text-xl font-bold text-white mb-2 group-hover:text-neon transition-colors line-clamp-2 relative z-10">
                    {post.title}
                </h3>

                {/* Excerpt */}
                <p className="text-sage/70 text-base line-clamp-3 leading-relaxed relative z-10">
                    {post.excerpt}
                </p>
              </div>

              {/* Footer */}
              <div className="flex items-center justify-between gap-4 text-sm text-sage/50 mt-auto border-t border-white/10 pt-4 relative z-10">
                <span className="flex items-center gap-1.5">
                  <Clock size={14} className="text-neon/70"/> 
                  {post.readTime}
                </span>
                {/* The Link Button */}
                <a 
                    className="flex items-center gap-2 text-sm font-bold text-white group-hover:text-neon transition-colors w-fit group/link"
                >
                    View Blog
